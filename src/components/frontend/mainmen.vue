<template>
  <ul class="main-nav" :class="$route.meta.headerMainNavClass">
    ...
    <li class="has-submenu megamenu" :class="homeMenu ? 'active' : 'notactive'">
      <a @click="toggleVisibility('home')">CASA <i class="fas fa-chevron-down"></i></a>
      <ul class="submenu mega-submenu" :style="{ display: isVisible.home ? 'block' : 'none' }">
        <li>
          <div class="megamenu-wrapper">
            <div class="row">
              <div class="col-lg-2" v-for="(item, index) in homeMenuItems" :key="index">
                <div class="single-demo" :class="currentPath === item.path ? 'active' : 'notactive'">
                  <div class="demo-img">
                    <router-link :to="item.path" class="inner-demo-img">
                      <img :src="item.image" class="img-fluid" alt="img" />
                    </router-link>
                  </div>
                  <div class="demo-info">
                    <router-link :to="item.path" class="inner-demo-img">{{ item.label }}</router-link>
                  </div>
                </div>
              </div>
            </div>
    ...
    <li class="has-submenu" :class="blogMenu ? 'active' : 'notactive'">
      ...
      <ul class="submenu" :style="{ display: isVisible.blog ? 'block' : 'none' }">
        <li v-for="(item, index) in blogMenuItems" :key="index" :class="currentPath === item.path ? 'active' : 'notactive'">
          <router-link :to="item.path">{{ item.label }}</router-link>
        </li>
      </ul>
    </li>

    <!-- Menú de Admin (solo visible para administradores) -->
    <li v-if="isAdmin" class="has-submenu" :class="adminMenu ? 'active' : 'notactive'">
      <a @click="toggleVisibility('admin')">Admin <i class="fas fa-chevron-down"></i></a>
      <ul class="submenu" :style="{ display: isVisible.admin ? 'block' : 'none' }">
        <li v-for="(item, index) in adminMenuItems" :key="index" :class="currentPath === item.path ? 'active' : 'notactive'">
          <router-link :to="item.path">{{ item.label }}</router-link>
        </li>
        <!-- Nuevo ítem para el mantenedor de especialidades -->
        <li :class="currentPath === '/admin/especialidades' ? 'active' : 'notactive'">
          <router-link to="/admin/especialidades">Mantenedor de Especialidades</router-link>
        </li>
      </ul>
    </li>
...
export default {
  data() {
    return {
      ...
      blogMenuItems: [
        { path: "/blog-list", label: "Blog List" },
  ...
  computed: {
    ...
    adminMenu() {
      return this.adminMenuItems.some((item) => item.path === this.currentPath) || 
             this.currentPath === '/admin/especialidades';
    },
  },
  methods: {
