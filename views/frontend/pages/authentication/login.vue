...
export default {
  ...
  methods: {
    async submitForm() {
      ...
      try {
        // Cambia la URL a /auth/login para coincidir con el backend
    ...
    handleLoginError(error) {
      ...
      if (error.response) {
        switch (error.response.status) {
          case 400:
            errorMessage = error.response.data.error || "Datos de inicio de sesión inválidos";
            break;
          case 401:
            errorMessage = error.response.data.error || "Credenciales incorrectas. Por favor, verifica tu email y contraseña";
            variant = "warning";
            break;
          case 403:
